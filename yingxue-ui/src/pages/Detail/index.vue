...
export default {
  ...
  computed: {
    ...
    //服务器数据没回来时，子组件skuImageList会是undefined，所以会报错
    skuImageList() {
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      //遍历全部属性值isChicked为零，没有高亮
      //arr、saleAttrValue 实参是个引用类型，之后的任何一方改变都会映射到另一方。
      arr.forEach((item) => {
        item.isChecked = "0";
      });
    ...
    async addShopcar() {
      //1、发请求，产品加入购物车（通知服务器）
      /**
              当前这里派发一个actions,也想服务器发请求，判断加入购物车是成功还是失败
              说白了就是：调用仓库中的addOrUpdateShopCart,返回是个promise。因为async函数执行返回的结果一定是个promise
            【要么成功，要么失败】
      */
      ...
      //param路径不会变
      // this.$router.push({name:'addcartsuccess',params:{skuInfo:this.skuInfo,skuNum:this.skuNum}});
      //简单数据通过 query传递
      //比较复杂数据，通过会话存储（不持久化，会话结束数据消失）
      //本地存储和会话存储不能存对象，一般存字符串
